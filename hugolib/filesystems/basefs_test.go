...
func TestRealDirs(t *testing.T) {
	...
	fi, b, err := bfs.theBigFs.overlay.(afero.Lstater).LstatIfPossible(filepath.Join("resources", "t1.txt"))
	assert.NoError(err)
	assert.False(b)
...
func countFileaAndGetFilenames(fs afero.Fs, dirname string) (int, []string, error) {
	...
	w := hugofs.NewWalkway(fs, dirname, func(info hugofs.FileMetaInfo, err error) error {
		...
		if info.Name() != "." {
			...
			filename = strings.Replace(filename, filepath.FromSlash("/my/work"), "WORK_DIR", 1)
